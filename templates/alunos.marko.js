...
function render(input, out, __component, component, state) {
  ...
  marko_forOf(data.results, function(aluno, index) {
    ...
    out.w("<tr><td>" +
      ...
      " class=\"btn-floating btn-small waves-effect waves-light red\"><i class=material-icons>create</i></a><a" +
  ...
  marko_forOf(data.error_messages, function(message, index) {
    out.w("<script>" +
      marko_escapeScript(("M.toast({html: '" + message) + "', classes:'red lighten-2'})") +
      "</script>");
  });

  marko_forOf(data.success_messages, function(message, index) {
    out.w("<script>" +
      marko_escapeScript(("M.toast({html: '" + message) + "', classes:'teal accent-4'})") +
      "</script>");
  });
